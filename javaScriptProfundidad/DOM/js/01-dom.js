...
contenidoTexto.innerHTML = 'Cambio de texto en front no rojo';
console.log('la que no es roja',contenidoTexto.className);
...
seccion.append(hr);// el hr es para hacer una línea de separación.
...
var divsAmarillos = document.querySelectorAll('.amarillo');// Este saca solo un elemento, no saca un array. pero el all regresa un nodeList
console.log('divs amarillos ',divsAmarillos);

divsAmarillos[0].style.background = 'grey';


for (var indice in divsRojos) {
	if (divsRojos[indice].className == 'rojo') {
		divsRojos[indice].style.background = 'blue';
	}
}
